...
1. `v4.1.3` 是当前采用的官方稳定版本基线。它是不可变的发布标签，只用于创建升级分支和进行差异比对，不直接提交开发代码。
2. `release-4.1` 是官方维护参考分支，只用于查看和选择性吸收官方修复；不得将其整体合并到定制分支，也不得在其上开发本项目功能。需要采用官方修复时，必须先确认与定制代码不存在冲突，再按提交或补丁粒度引入并测试。
3. 当前 `master` 仍是旧的 v4.1.1 系列，在 v4.1.3 迁移完成前冻结：不再从它创建新的功能分支，不直接提交功能代码，也不把未验证的阶段成果合入其中。现有功能仍需保留时，先建立归档分支作为回滚依据。
4. `feature/oci-performance-cache` 是当前已验证的功能基线。v4.1.4 候选从该分支创建同名的 `upgrade/v4.1.4-oci`，再按提交/文件粒度选择性吸收官方 `v4.1.3` 修复；不得整体合并官方发布分支，也不再继续扩展旧的 `upgrade/v4.1.3-oci`。
5. 迁移切换前保留以下归档引用（仅用于回滚和对比，不再继续开发）：`archive/master-v4.1.1` 指向切换前的 `master`，`archive/oci-v4.1.1` 指向当前 OCI 定制线，`upgrade/v4.1.3-oci` 保留为失败迁移的审计参考。切换不得通过无记录的强制覆盖；如远端分支受保护，应通过一次可审计的合并或管理员批准完成。
6. 新功能分支统一从最新且已验证的 `master` 创建，命名为 `feature/<主题>`；短期修复使用 `fix/<主题>`。功能完成后必须经过测试和测试环境验证，再合入 `master`；未完成工作不得直接进入 `master`。
7. 前后端使用同名分支和同一阶段标识，例如后端与 Console 都使用 `feature/oci-performance-cache`。两边可以分别提交，但必须分别完成构建、部署和验证后再合并，不能只更新一端。
8. 每次官方版本升级都创建新的 `upgrade/vX.Y.Z` 分支，从对应官方发布标签开始迁移；不得通过修改默认版本字符串伪装升级，也不得修改或重写官方标签。
9. 本项目是独立维护的发行版。完成 `upgrade/v4.1.4-oci` 的选择性迁移和定制功能验证后，可以发布项目自有的 `v4.1.4`；该版本必须从新的项目 `master` 产生，不能直接从仍基于 v4.1.1 的旧功能分支打包。
10. 项目版本发布使用发布分支 `release/vX.Y.Z`，先完成构建、测试、测试环境验证和回滚检查，再创建不可变 Git 标签 `vX.Y.Z` 并发布 GitHub Release 和对应镜像。候选版本使用 `vX.Y.Z-rc.N`；不得把开发分支或临时构建标签当作正式版本。
11. 项目自有版本与上游版本必须可区分。若未来上游发布同名版本，项目定制版本优先使用 `vX.Y.Z-mysstack.N` 或在发布说明中明确标注为 KubeSphere Fork 定制版，不覆盖上游标签。
...
`release-4.1` 只作为上游参考线，不能替代发布标签，也不能替代本项目的 `master`。任何来自上游的修复都要经过选择、冲突检查和本项目测试后才能进入定制分支。

### 项目自有版本发布流程

```text
feature/oci-performance-cache（已验证功能基线）
    │
    └─ upgrade/v4.1.4-oci（选择性吸收 v4.1.3 并验证）
                                  │
                                  └─ master（项目新基线）
                                       │
                                       └─ release/v4.1.4
...
- 官方稳定基线：`v4.1.3`（修复和兼容性参考）
- 官方维护参考：`release-4.1`（只读参考，不作为功能开发分支）
- 当前后端分支：`feature/oci-performance-cache`
- 当前前端分支：`feature/oci-performance-cache`
- 当前 `master`：仍为 v4.1.1 系列，迁移完成前冻结
- 当前候选分支：`upgrade/v4.1.4-oci`（从 `feature/oci-performance-cache` 创建，尚未完成验证）
