...
4. 从 `v4.1.3` 创建一次性迁移分支 `upgrade/v4.1.3-oci`，将当前已验证的 OCI、私有仓库凭据、同步缓存/限流、401 处理和 CI 定制逐项迁移到该基线。迁移分支必须完成编译、测试和测试环境验证后，才能成为新的 `master` 基线。
5. 迁移切换前保留以下归档引用（仅用于回滚和对比，不再继续开发）：`archive/master-v4.1.1` 指向切换前的 `master`，`archive/oci-v4.1.1` 指向当前 OCI 定制线。切换不得通过无记录的强制覆盖；如远端分支受保护，应通过一次可审计的合并或管理员批准完成。
...
9. 本项目是独立维护的发行版。完成 v4.1.3 基线迁移和定制功能验证后，可以发布项目自有的 `v4.1.4`；该版本必须从新的项目 `master` 产生，不能直接从仍基于 v4.1.1 的旧功能分支打包。
